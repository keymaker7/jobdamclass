...
const schema=z.object({jobId:z.string(),question:z.string().trim().min(1).max(600),purpose:z.string().max(500),history:z.array(z.object({question:z.string().max(600),answer:z.string().max(6000)}).strict()).max(20),connection:z.object({apiKey:keySchema,model:modelSchema,teacherPreview:z.literal(true)}).strict().optional()}).strict();
const pii=/\b01[016789][ -]?\d{3,4}[ -]?\d{4}\b|\b\d{6}[ -]?[1-4]\d{6}\b|[\w.+-]+@[\w.-]+\.[a-z]{2,}|sk-[A-Za-z0-9_-]{12,}/i;
