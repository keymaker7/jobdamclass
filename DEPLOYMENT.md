...
기본은 꺼져 있습니다. 아래 세 값이 **모두** 있을 때만 켜지고, 하나라도 비면 학생은 지금처럼 상황 답변으로 면담합니다.
**끄는 방법은 `CLASS_AI_KEY`를 지우고 재배포하는 것**입니다. 코드를 되돌릴 필요가 없습니다.
다만 그건 앱에서 못 쓰게 막는 것일 뿐이니, 키를 완전히 죽이려면 발급처에서 재발급(rotate)하세요.

| 변수 | 필수 | 설명 |
|---|---|---|
| `CLASS_AI_KEY` | ✅ | 발급받은 키. **서버에만 있고 브라우저로 내려가지 않습니다.** |
| `CLASS_AI_MODEL` | ✅ | `lib/ai-models.ts`의 모델 id. 공급자는 여기서 자동으로 정해집니다. |
| `CLASS_CODE` | ✅ | 수업 코드. 아이들은 `?class=<코드>`가 붙은 주소로 들어옵니다. |
| `CLASS_AI_PER_LEARNER` | | 한 사람이 하루에 쓸 수 있는 횟수 (기본 5) |
| `CLASS_AI_PER_CLASS` | | 수업 코드 하나가 하루에 쓸 수 있는 총 횟수 (기본 200) — **비용 천장** |

`DATABASE_URL`과 `SESSION_SECRET`도 함께 필요합니다. 사용 횟수를 세는 표와 코드 해시에 쓰입니다.

수업 때 아이들에게 주는 주소:

```
https://<도메인>/?class=<CLASS_CODE>
```

코드는 들어오는 즉시 주소창에서 지워지고 그 탭에만 남습니다. 공용 기기에서는 **끝내기**를 누르면 수업 코드와 기기 id가 함께 지워집니다.

### 이 설정으로 막는 것

- 코드가 없거나 틀리면 AI 호출 자체가 일어나지 않습니다 (401 / 403). 주소만 새어도 밖에서 못 씁니다.
- 자해·위험 질문과 합격선·연봉 질문은 **AI를 부르기 전에** 정해진 안내로 답합니다. 이 질문들은 요금이 나가지 않습니다.
- 하루 상한에 닿으면 오류를 내지 않고 상황 답변으로 자연스럽게 이어집니다. 수업 중에 아이가 막다른 화면을 보지 않습니다.

`GET /api/interview` (교사 로그인 필요)로 오늘 사용량과 설정된 상한을 확인할 수 있습니다.
